document.getElementById("form1").addEventListener("submit", function (event) {
  ...
  if (nome.split(" ").length < 2 || nome.length < 6) {
    valido = false;
    mensagem.push(
      "O nome deve conter pelo menos duas palavras e ter no mínimo seis caracteres"
    );
  }
  ...
  if (!valido) {
    /* ! "== false" */ event.preventDefault(); /* impede o envio do formulário */
    alert(mensagem.join("\n"));
  }
});
